...
Public NotInheritable Class Vector4Editor
    Inherits UserControl

    Public Property Value As Vector4
        Get
    ...
    Public Shared ReadOnly ValueProperty As DependencyProperty =
                           ...
                           GetType(Vector4), GetType(Vector4Editor),
                           New PropertyMetadata(New Vector4,
                                                Sub(sender, e)
                                                    Dim editor = DirectCast(sender, Vector4Editor)
                                                    Dim valuePreview = editor.TblMatrix
                                                    Dim vec = DirectCast(e.NewValue, Vector4)
                                                    valuePreview.Text = $"<{vec.X},{vec.Y},{vec.Z},{vec.W}>"
                                                    editor.ReloadValue()
                                                End Sub))
    ...
    Private Sub Flyout_Closing(sender As FlyoutBase, args As FlyoutBaseClosingEventArgs)
        TblError.Text = ""
        Try
            Value = New Vector4(TxtX.Text, TxtY.Text,
                                TxtZ.Text, TxtW.Text)
            _isCanceling = False
